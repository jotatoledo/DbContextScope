...
namespace EntityFrameworkCore.DbContextScope
{
    ...
    public class DbContextScope : IDbContextScope
    {
        ...
        public async Task<int> SaveChangesAsync(CancellationToken cancelToken = default(CancellationToken))
        {
            this.ThrowIfDisposed();
            this.ThrowIfCompleted();
        ...
        public void RefreshEntitiesInParentScope(IEnumerable entities)
        {
            ...
            {
                if (this.parentScope.dbContexts
                    .InitializedDbContexts
                    .Values
                    .SingleOrDefault(ParentContextWithMatchingType(contextInCurrentScope)) is DbContext correspondingParentContext)
                {
...
" + Environment.StackTrace;
        ...
        private static Func<DbContext, bool> ParentContextWithMatchingType(DbContext contextInCurrentScope)
        {
            return parentContext => parentContext.GetType() == contextInCurrentScope.GetType();
        }
        ...
        private void ThrowIfCompleted()
        {
            if (this.completed)
            {
                throw new InvalidOperationException(
                    ...
                    A DbContextScope is meant to encapsulate a business transaction: create the scope at the start of the business transaction and then call SaveChanges() at the end. 
                    Calling SaveChanges() mid-way through a business transaction doesn't make sense and most likely mean that you should refactor your service method into two separate service method that each create their own DbContextScope and each implement a single business transaction.");
            }
        }
        ...
        /// </summary>
        /// <param name="newAmbientScope">The ambient scope</param>
        internal static void SetAmbientScope(DbContextScope newAmbientScope)
        {
            ...
            var current = ContextScopeCallContext.GetData(AmbientDbContextScopeKey) as InstanceIdentifier;
            ...
            ContextScopeCallContext.SetData(AmbientDbContextScopeKey, newAmbientScope.instanceIdentifier);
        ...
        {
            ContextScopeCallContext.SetData(AmbientDbContextScopeKey, null);

            if (ContextScopeCallContext.GetData(AmbientDbContextScopeKey) is InstanceIdentifier current)
            {
        ...
        /// </summary>
        /// <returns>The current ambient scope or null if no ambient scope has been setup</returns>
        internal static DbContextScope GetAmbientScope()
        {
            // Retrieve the identifier of the ambient scope (if any)
            var instanceIdentifier = ContextScopeCallContext.GetData(AmbientDbContextScopeKey) as InstanceIdentifier;
