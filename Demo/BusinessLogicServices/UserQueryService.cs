...
namespace Numero3.EntityFramework.Demo.BusinessLogicServices
{
    ...
    public class UserQueryService
    {
        ...
        public async Task<IList<User>> GetTwoUsersAsync(Guid userId1, Guid userId2)
        {
            ...
            using (this.dbContextScopeFactory.CreateReadOnly())
            {
                var user1 = await this.userRepository.GetAsync(userId1).ConfigureAwait(false);
                ...
                var user2 = await this.userRepository.GetAsync(userId2).ConfigureAwait(false);
        ...
        public User GetUserUncommitted(Guid userId)
        {
             ...
             */
            using (this.dbContextScopeFactory.CreateReadOnlyWithTransaction(IsolationLevel.ReadUncommitted))
            {
